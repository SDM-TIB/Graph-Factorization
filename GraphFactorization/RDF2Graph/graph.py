...
class Graph2(object):
    ...
    def add_vertex(self, vid, vlb):
        ...
        #print(self.vertices[vid].get_vlb(vid))
                ...
                if self.is_undirected:
                    if frm < to:
                        ...
                        display_str += 'e {} {} {} '.format(
                            frm, to, edges[to].elb)
                        display_str += "\n"
                        #print('({},{})'.format(edges[to].elb, self.vertices[to].vlb))
                else:
                    #print('e {} {} {}'.format(frm, to, edges[to].elb))
    ...
    def plot(self):
        ...
        try:
            ...
            return
        gnx = nx.Graph2() if self.is_undirected else nx.DiGraph()
            ...
            for to in edges:
                ...
                if self.is_undirected:
                    if frm < to:
                        ...
                        #display_str += 'e {} {} {} '.format(frm, to, edges[to].elb)
                        #print('({},{})'.format(edges[to].elb, self.vertices[to].vlb))
                        #poPair.add(edges[to].elb+self.vertices[to].vlb)
                        poDict[self.vertices[frm].vlb].add(edges[to].elb+self.vertices[to].vlb)
                else:
                    #print('e {} {} {}'.format(frm, to, edges[to].elb))
                    #display_str += 'e {} {} {}'.format(frm, to, edges[to].elb)
                    #print('({},{})'.format(edges[to].elb, self.vertices[to].vlb))
    ...
    def get_properies_POPairs(self):
        poSet = set()
        propSet = set()
        #print('t # {}'.format(self.gid))
        #for vid in self.vertices:
            ...
            for to in edges:
                ...
                if self.is_undirected:
                    if frm < to:
                        ...
                        #poPair.add(edges[to].elb+self.vertices[to].vlb)
                        #poDict[self.vertices[frm].vlb].add(edges[to].elb+self.vertices[to].vlb)
                        poSet.add((edges[to].elb, self.vertices[to].vlb))
                        propSet.add(edges[to].elb)
                else:
                    #print('e {} {} {}'.format(frm, to, edges[to].elb))
                    #display_str += 'e {} {} {}'.format(frm, to, edges[to].elb)
                    #print('({},{})'.format(edges[to].elb, self.vertices[to].vlb))
                    ...
                    poSet.add((edges[to].elb, self.vertices[to].vlb))
                    propSet.add(edges[to].elb)
        return poSet, propSet


    def get_po_dict(self):
        return

class sourcePO(object):
    """Edge class."""
    def __init__(self,
        ...
        Args:
             elbl: edge label.
             tolbl: destination vertex label.
             """
        self.source = source
        self.poPair = set()

    def add_po_pair(self, poPair):
        self.poPair.add(poPair)
